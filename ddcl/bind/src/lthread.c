...
l_sleepms(lua_State * L){
	dduint32 ms = luaL_checkinteger(L, 1);
	ddcl_sleepms(ms);
	return 0;
}

static luaL_Reg _REG[] = {
	{ "new_thread", l_new_thread },
	{ "cancel_thread", l_cancel_thread },
	{ "join_thread", l_join_thread },
	{ "sleepms", l_sleepms },
    { NULL, NULL },
};
...
CLLUA int
luaopen_lddcl_thread (lua_State * L){
    luaL_checkversion(L);
    DDLUA_NEWLIB(L, "lddcl.thread", _REG);
    return 1;
}
